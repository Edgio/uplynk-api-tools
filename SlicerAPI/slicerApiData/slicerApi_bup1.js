...
/// CREATE TABLE ELEMENTS ///
    function createTableElements(data, classID, header, nested, encoder) {
        ...
        if (nested === 0) {
            ...
            const headerKey = headerRow.insertCell(0);
                ...
                headerKey.colSpan = 2; // add this line to span the cell across two columns
    ...
    function showHideUIElements(clicked, checked) {
        const checkboxNames = tableNames.map((table) => `${table}`);
        const checkboxIndex = checkboxNames.indexOf(clicked);
        if (checkboxIndex !== -1) {
            const table = document.getElementsByClassName(`_${tableNames[checkboxIndex]}`);
    ...
    statsOptions.addEventListener('click', function (e) {
        if (e.target.type === 'checkbox') {
            ...
            showHideUIElements(clicked, checked);
        }
    });
    ...
    var encoder0 = '';
    var encoder1 = '';
    // declareEncoders('encoder0', 'https://ingest-prod-1-eu-central-1.mss.aws.oath.cloud:48438'); //   TEMPORARY >>>>>>>>>>>>>>>>>>>>>>>
    // declareEncoders('encoder1', 'https://ingest-prod-0-eu-central-1.mss.aws.oath.cloud:46343'); //   TEMPORARY >>>>>>>>>>>>>>>>>>>>>>>
        ...
        for (var i = 0; i < encodersKeys.length; i++) {
            ...
            if (encoders[encodersKeys[i]] != '') {
                ...
                async function runCallAPI(index) {
                    ...
                    const masterCatObj = ['stateObj', 'imageSoundObj', 'timeObj', 'frameObj', 'sliceProcessingObj', 'connectionObj', 'miscObj'];
                    ...
                    cl(response);
                    if (response.error === 0) {
                        masterCatObj.forEach(function(categoryItem) {
                            let category = {};
                            let count;
                            let categoryValues;
                                category[categoryItem] = {};
                                categoryValues = eval(categoryItem); // get the array of values for this category
                                for (count = 0; count < categoryValues.length; count++) {
                                    if (count === 0) {
                                    } else {
                                        if (categoryValues[count] === 'state') {
                                            // cl('response.status[categoryValues[count]] === state');
                                            // cl(response.status[categoryValues[count]]);
                                            // cl(slicerStateArray[response.status[categoryValues[count]]]);
                                            let object = {};
                            ...
                            // cl(category);
                            let table = createTableElements(category[categoryItem], '_' + categoryItem.replace('Obj', ''), eval(categoryItem)[0], 0, encoderIndex);
            ...
            if (encoders[encodersKeys[i]] != '') {
                ...
                async function runCallAPI(index) {
                    ...
                    if (response.error === 0) {
                        ...
                        let table = createTableElements(qualityCategory, '_quality', 'Quality', 0, encoderIndex);
                        let tableClass = table.classList;
                        if (statsVisibility['qualityTable'] === false) {
                            table.style.display = 'none';
                        }
                        statsDiv[index].appendChild(table);
                    }
        ...
        if (e.target.id === 'contentStartButton' || e.target.id === 'blackoutButton' || e.target.id === 'manualRefresh') {
            ...
            if (encoder !== null) {
                if (encoder !== '') {
                    ...
                    uiEncoderVal = uiEncoderVal.replace('.aws.oath.cloud:', ' : ')
            ...
            apiKey = prompt("Enter Encoder API KEY:");
